...
// 
// This is part of revision 5570 of the EK-LM3S3748 Firmware Package.
//
//*****************************************************************************
...
ClassDPWMHandler(void)
{
    ...
    //
    ROM_PWMGenIntClear(PWM_BASE, PWM_GEN_1, PWM_INT_CNT_ZERO);
    ...
    if(HWREGBITW(&g_ulClassDFlags, CLASSD_FLAG_STARTUP))
    {
        ...
        //
        ROM_PWMDeadBandEnable(PWM_BASE, PWM_GEN_1, 0, g_ulClassDStep);
        ROM_PWMPulseWidthSet(PWM_BASE, PWM_OUT_2,
    ...
    if(HWREGBITW(&g_ulClassDFlags, CLASSD_FLAG_SHUTDOWN))
    {
        ...
        if(g_ulClassDStep == (g_ulClassDPeriod - 2))
        {
            ...
            //
            ROM_PWMOutputState(PWM_BASE, PWM_OUT_2_BIT | PWM_OUT_3_BIT, false);
    ...
    ROM_PWMPulseWidthSet(PWM_BASE, PWM_OUT_2, lDutyCycle);
...
ClassDInit(unsigned long ulPWMClock)
{
    ...
    ROM_PWMGenPeriodSet(PWM_BASE, PWM_GEN_1, g_ulClassDPeriod);

    //
    // Start the PWM outputs with one clock long pulses.
    //
    ROM_PWMPulseWidthSet(PWM_BASE, PWM_OUT_2, 1);
    ROM_PWMDeadBandEnable(PWM_BASE, PWM_GEN_1, 0, g_ulClassDPeriod - 2);
    ...
    //
    ROM_PWMGenIntTrigEnable(PWM_BASE, PWM_GEN_1, PWM_INT_CNT_ZERO);
    ROM_PWMIntEnable(PWM_BASE, PWM_INT_GEN_1);
}
